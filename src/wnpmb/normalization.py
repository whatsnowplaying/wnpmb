...
STRIPRELIST: list[re.Pattern[str]] = [
    re.compile(r" \((?:{})\)".format("|".join(SORTED_STRIPWORDLIST)), re.IGNORECASE),
    re.compile(r" - (?:{}$)".format("|".join(SORTED_STRIPWORDLIST)), re.IGNORECASE),
    re.compile(r" \[(?:{})\]".format("|".join(SORTED_STRIPWORDLIST)), re.IGNORECASE),
]
...
def build_recording_query(
    ...
    if strict:
        parts.append("-(secondarytype:Compilation OR secondarytype:Live)")
...
def extract_tags_from_data(data: dict, source_name: str) -> list[str] | None:
    ...
    if not data or not data.get("tags"):
        return None
    tags = [t["name"] for t in data["tags"] if "name" in t]
    if tags:
...
def extract_genres(mb_data: dict) -> list[str] | None:
    ...
    names = [g["name"] for g in sorted_genres if "name" in g]
    if names:
...
def select_best_release(
    ...
    for release in pool:
        ...
        # Prefer studio albums over compilations/live albums/singles/EPs
        ...
        if recording_title and release.get("title"):
            if normalize(recording_title) == normalize(release["title"]):
                score += 20
